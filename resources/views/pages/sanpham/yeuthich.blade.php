...
@section('content_thu2')
    <div class="features_items">
        ...
        <div class="row">
            @foreach ($wishlistItems as $yt)
                @if(isset($yt->product))
                    <div class="col-md-3">
                        <a href="{{ URL::to('chi-tiet-san-pham/' . $yt->product->product_slug) }}">
                            <div class="product-image-wrapper">
                                <div class="single-products">
                                    <div class="productinfo text-center">
                                        <div class="position-relative">
                                            <img id="wishlist_productimage{{ $yt->product->product_id }}"
                                                 src="{{ URL::to('public/uploads/product/' . $yt->product->product_image) }}"
                                                 alt=""/>
                                            @if ($yt->product->product_km != 0)
                                                <a style="color:white" class="discount-badge">
                                                    -{{ round((($yt->product->product_km - $yt->product->product_price) / $yt->product->product_km) * 100) }}%
                                                </a>
                                            @endif
                                        </div>
                                        <p style="margin-top: 20px">{{ $yt->product->product_name }}</p>
                                        @if ($yt->product->product_km != 0)
                                            <div>
                                                <h5 style="display: inline; text-decoration: line-through;">
                                                    {{ number_format($yt->product->product_km, 0, ',', '.') }} VNĐ
                                                </h5>
                                                <h4 style="display: inline;color:red">
                                                    {{ number_format($yt->product->product_price, 0, ',', '.') }} VNĐ
                                                </h4>
                                            </div>
                                        @else
                                            <h4 style="color: red;">
                                        ...
                                        @endif
                                        <p style="margin-top: 15px;color:brown">Đã bán:{{ $yt->product->product_sold }}</p>
                                        <input type="button" value="Xem sản phẩm"
                                               class="btn btn-danger btn-sm add-to-cart"
                                               data-id_product="{{ $yt->product->product_id }}" name="add-to-cart">
                                    </div>
                                </div>
                            </div>
                        </a>
